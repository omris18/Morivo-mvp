...
export default function Runtime({experience,setView}){
 ...
 <div className="grid2" style={{marginTop:18}}><div className="panel"><div className="tag">Live Activity</div><div className="feed">{feed.map((x,i)=><div key={i}>{x}</div>)}</div></div><div className="panel"><div className="tag">Latest Memories</div>{media.length?<div className="runtimeMedia">{media.slice(0,6).map(m=>m.contentType?.startsWith("video/")?<video key={m.id} src={m.downloadURL} muted/>:<img key={m.id} src={m.downloadURL} alt="memory"/>)}</div>:<p>No photos yet.</p>}</div></div>
