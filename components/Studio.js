...
export default function Studio({experience,setExperience,setView}){
 ...
 function patch(values){persist({...experience,flow:flow.map(x=>x.id===atom.id?{...x,...values}:x)})}
 function add(type){const n={id:type+"-"+Date.now(),type,title:"New "+type,text:"Describe what participants should do.",reward:""};persist({...experience,flow:[...flow,n]});setSelected(n.id)}
 ...
 return <section className="grid2">
  <div className="panel">
   ...
   {atom&&<div className="inspector"><label>Title</label><input value={atom.title} onChange={e=>patch({title:e.target.value})}/><label>Participant instruction</label><textarea value={atom.text} onChange={e=>patch({text:e.target.value})}/><label>Reward</label><input value={atom.reward||""} onChange={e=>patch({reward:e.target.value})}/></div>}
  ...
  <div className="panel"><div className="tag">Live participant preview</div>{atom&&<div className="phone"><h3>{atom.title}</h3><p>{atom.text}</p><div className="mission">{atom.reward?`Reward: ${atom.reward}`:"No reward"}</div><button className="primary">Complete Mission</button></div>}</div>
 </section>
}
