...
export default function Participant({experience,setExperience,setView,setActiveId,deepLinkCode,t}){
 ...
 useEffect(()=>{setQuizAnswer(null);setNoteText("");setPuzzleAnswer("");setLocStatus(null)},[idx]);
 ...
 async function complete(){if(!mission)return;const isMedia=mission.type==="photo"||mission.type==="video";if(isMedia&&!file)return alert(p.chooseFirst(mission.type));if(mission.type==="quiz"&&!quizAnswer)return alert(p.chooseAnswerFirst);if(mission.type==="note"&&!noteText.trim())return alert(p.writeSomethingFirst);if(mission.type==="puzzle"&&mission.answer&&puzzleAnswer.trim().toLowerCase()!==String(mission.answer).trim().toLowerCase())return alert(p.wrongAnswer);if(mission.type==="map"&&hasCheckpoint&&!(locStatus&&locStatus.within))return alert(p.getCloserFirst);setBusy(true);try{
 ...
 {mission.type==="quiz"&&<div className="choiceGrid">{["A","B","C"].map(opt=><button key={opt} className={quizAnswer===opt?"selected":""} onClick={()=>setQuizAnswer(opt)}>{opt}</button>)}</div>}{mission.type==="map"&&(hasCheckpoint?<div className="mapMock gpsReal">📍<span>{locStatus==="checking"?p.checkingLocation:locStatus?.within?p.arrived:locStatus?p.metersAway(Math.round(locStatus.distance)):p.getToCheckpoint}</span><button type="button" disabled={locStatus==="checking"} onClick={checkLocation}>{p.checkMyLocation}</button></div>:<div className="mapMock">📍<span>{p.locationCheckpoint}</span></div>)}{mission.type==="puzzle"&&<div className="puzzleBox">🧩<input value={puzzleAnswer} placeholder={p.puzzleAnswerPlaceholder} onChange={e=>setPuzzleAnswer(e.target.value)}/></div>}
 ...
 {busy&&(mission.type==="photo"||mission.type==="video")&&<div className="uploadProgress"><div style={{width:`${pct}%`}}></div><span>{pct}%</span></div>}<div className="mission">{p.reward}: {mission.reward||`${mission.points||100} pts`}</div><button className="primary" disabled={busy} onClick={complete}>{busy?p.saving:p.completeContinue}</button></div>}</>}
