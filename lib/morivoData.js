...
export async function completeJourneyMission(experienceId,mission,index,name="Participant",nextIndex=null){
  ...
  // A branch mission jumps to whichever mission its chosen option targets, not necessarily
  // index+1 - nextIndex carries that target when the caller resolved one.
  const resolvedIndex=Number.isFinite(nextIndex)?nextIndex:index+1;
  const next={uid:user.uid,participantName:name,completedMissionIds:Array.from(new Set([...(old.completedMissionIds||[]),mission.id])),
    currentMissionIndex:Number.isFinite(nextIndex)?resolvedIndex:Math.max(old.currentMissionIndex||0,resolvedIndex),points:(old.points||0)+(already?0:Number(mission.points||100)),
