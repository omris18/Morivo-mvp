...
export function subscribeParticipantCodes(experienceId, callback) {
  if (!firebaseConfigured || !experienceId) return () => {};
  const q = query(collection(db, "participantCodes"), where("experienceId", "==", experienceId));
  return onSnapshot(q, snap => callback(snap.docs.map(d => ({ code: d.id, ...d.data() }))));
}
...
export async function joinExperienceByPersonalCode(pcode) {
  ...
  const user = await ensureUser();
  await setDoc(doc(db, "experiences", data.experienceId, "participants", user.uid), {
  ...
  return { experienceId: data.experienceId, name: data.name, uid: user.uid };
}
